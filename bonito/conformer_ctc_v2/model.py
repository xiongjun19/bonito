...
def transformer_encoder(n_base, state_len, insize=1, stride=5, winlen=19, activation='swish', rnn_type='lstm', features=768, scale=5.0, blank_score=None, expand_blanks=True, num_layers=5, att_out_dim=128, dropout=0.1):
    n_heads = features // 64
    return Serial([
...
class AttLinear(nn.Module):
    ...
    def forward(x):
        ...
        h = self.activation(h)
        B, T, H = h.size()
        res = h.view([B, T * self.tag_num, self.out_dim])
        return res
